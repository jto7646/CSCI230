...
public class LinkedPositionalList<E> implements PositionalList<E>, Cloneable {
  ...
  public LinkedPositionalList<E> clone() throws CloneNotSupportedException {
    ...
    if(size() > 0){
      // Make a new head node for the clone for deep clone 
      other.header = new Node(null, null , null);
      Node<E> walk = header.getNext(); // Used to itterating through the original list
      Node<E> otherTemp = other.header; // Starting point for the copied list
      // Itterates through the original list, copying nodes into the cloned list
      while(walk.getNext() != null){
        Node<E> copy = new Node(walk.getElement(), otherTemp, walk.getNext()); // Node copied from original 
        otherTemp.setNext(copy); // copy list pointing to new node
        otherTemp = copy;
        walk = walk.getNext();
      }
    }
  ...
  // Bellow is the test main function
  /*
  public static void main(String[] args) {
    LinkedPositionalList posList = new LinkedPositionalList<Integer>();
    LinkedPositionalList posClone = new LinkedPositionalList<Integer>();

    posList.addAfter(posList.header, 12);
    posList.addAfter(posList.header, 15);
    posList.addAfter(posList.header, 24);

    System.out.print(posList.toString());

    try {
      posClone = posList.clone();
    } catch (Exception e) { System.out.println(e);   }

    System.out.println("\nCopied List:\n" + posClone.toString());

    posClone.remove(posClone.header.getNext());
    posClone.remove(posClone.header.getNext());

    System.out.print("Original after remove: " + posList.toString());

    System.out.println("\nCopied List after remove: " + posClone.toString());
