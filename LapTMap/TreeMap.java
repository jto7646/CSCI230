...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  /**
   * Returns the position at the desired index within the inorder traversal of the tree
   * @param i desired index within inorder traversal
   * @return Position within index, or null if index is invalid
   */
  public Position<Entry<K,V>> atIndex(int i){
    int indexCounter = 0;
    for (Position<Entry<K,V>> entry : tree.inorder()) {
      if(indexCounter == i){return entry;}
      indexCounter++;
    }
    return null;
  }
  ...
  /**
   * Returns the index of the desired position within the inorder traversal of the tree
   * @param pos position of which the index is being found
   * @return the index of the position, -1 if position is not in tree
   */
  public int indexOf(Position<Entry<K,V>> pos){
    int indexCounter = 0;
    for (Position<Entry<K,V>> entry : tree.inorder()) {
      if(pos == entry){return indexCounter;}
      indexCounter++;
    }
    return -1;
  }
