...
public class UDGraph {
	 ...
	 * number of vertices in the graph. edges is the number of edges in the graph.
	 *
	...
	/**
	 * Creates the edge (origin, destination). If the edge did not already exists,
	...
	/**
	 * Deletes the edge (origin, destination). If the edge existed, decrements the
	 * edge count.
	 * 
	 * @param origin      the origin vertex.
	 * @param destination the destination vertex.
	...
	/**
	 * Returns a new UDGraph with the same vertices as "this" UDGraph. The new graph
	 * has an edge (v, w) if and only if there is a path of length 2 from v to w in
	 * "this" graph.
	 * 
	...
	public UDGraph length2Paths() {
		...
		int[][] tempGraph = new int[vertices][vertices];
		int[] rowArr = new int[vertices];
		int[] colombArr = new int[vertices];
		int sum = 0;

		// Squares the original graphs adjMatrix
		for (int row = 0; row < vertices; row++) {
			// Fill array with row
			for (int i = 0; i < vertices; i++) {
				if(adjMatrix[row][i] == true){ rowArr[i] = 1;}
				else{ rowArr[i] = 0;}
			}

			for (int colomb = 0; colomb < vertices; colomb++) {
				sum = 0;
				// Fill array with colomb
				for (int g = 0; g < vertices; g++) {
					if(adjMatrix[g][colomb] == true){colombArr[g] = 1;}
					else{colombArr[g] = 0;}
				}	

				// Compute dot product
				for (int dot = 0; dot < vertices; dot++) {
					sum += rowArr[dot]*colombArr[dot];
				}
				// Save to temp array
				tempGraph[row][colomb] = sum;
			}
		...
		for (int i = 0; i < vertices; i++) {
			for (int j = 0; j < vertices; j++) {
				if(tempGraph[i][j] == 1){newGraph.addEdge(i, j);}
			}
		}
	...
	/**
	 * Returns a new UDGraph with the same vertices as "this" UDGraph. The new graph
	 * has an edge (v, w) if and only if there is a path of length "length" from v
	 * to w in "this" graph.
	 * 
	...
	/**
	 * Returns a String representing the adjacency matrix, including the number of
	 * vertices and edges.
	 * 
